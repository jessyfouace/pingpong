    ...
    // Check if pseudo got good write
      if (preg_match("#[a-z0-9._-]#", $_POST['connexionpseudo'])) {
        ...
        if (preg_match("#[a-z0-9._-]#", $_POST['connexionpassword'])) {
          ...
          $create = $bdd->query('SELECT * FROM admins');
          $create = $create->fetchAll();
          // foreach of bdd for take the user name...
          foreach ($create as $key => $value) {
            // if the connect is good create session
            if ($_POST['connexionpseudo'] == $value['admin_name'] AND password_verify($_POST['connexionpassword'], $value['admin_password'])) {
              $_SESSION['pseudo'] = $value['admin_name'];
              $_SESSION['mdp'] = password_verify($_POST['connexionpassword'], $value['admin_password']);
